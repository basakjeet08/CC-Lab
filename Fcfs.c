...
void scheduleProcess(struct Process process[] , int number){
    for(int i = 0 ; i < number ; i++){
        int lowest = i;
...
double runProcess(struct Process process[] , int number){
    ...
    return totalWaitingTime / number;
}
...
int main(){
    ...
    scheduleProcess(process , number); // Scheduling or Sorting the process according to Arrival Time
